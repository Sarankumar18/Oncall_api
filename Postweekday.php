...
elseif (($hour >= 14) && ($hour <22)) {
    $sql_L = "SELECT LateSL FROM Roster WHERE Date='$cur_date'";
    $result = $conn->query($sql_L);
    if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
      $SL = $row["LateSL"];
...
//Night
elseif ((($hour >= 22) && ($hour <= 24)) || (($hour >= 0) && ($hour <= 5))) {
    $sql_N = "SELECT NightSL FROM Roster WHERE Date='$cur_date'";
